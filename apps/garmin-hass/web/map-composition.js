...
export function setMapView(view) {
    if (host?.map) {
        if (host.pendingView !== null) {
            host.fail('simultaneous worker maps are not supported');
            return;
        }
...
export function startComposition(canvas, mode, repaint, map = false) {
    ...
    window.garminMapComposition = Object.freeze({ status: compositionStatus, dispose: disposeComposition });
}
