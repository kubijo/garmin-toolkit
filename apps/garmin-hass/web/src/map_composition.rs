//! Browser worker-map integration and its standalone composition fixture.

use eframe::{egui, wgpu};
use garmin_ui::activity::map_composition::{CompositionPlugin, Host, Placement};
use wasm_bindgen::prelude::*;

#[path = "map_composition/renderer.rs"]
pub(super) mod renderer;

#[wasm_bindgen(module = "/map-composition.js")]
extern "C" {
    ...
    #[wasm_bindgen(js_name = rendererMode, catch)]
    fn renderer_mode(enabled: bool, search: &str) -> Result<String, JsValue>;
...
pub(super) fn mode(canvas: &web_sys::HtmlCanvasElement) -> Result<String, JsValue> {
    let enabled = canvas
        .get_attribute("data-map-render-worker")
        .ok_or_else(|| super::js_error("the map render worker configuration is missing"))?;
    let enabled = serde_json::from_str::<bool>(&enabled)
        .map_err(|_| super::js_error("the map render worker configuration is invalid"))?;
    let search = web_sys::window()
        .ok_or_else(|| super::js_error("browser window is missing"))?
        .location()
        .search()?;
    renderer_mode(enabled, &search)
}

pub(super) fn options(mode: &str) -> eframe::WebOptions {
    let mut options = eframe::WebOptions::default();
    if !mode.is_empty() {
        // Use the same WebGL2 UI backend for main-thread and worker map modes.
