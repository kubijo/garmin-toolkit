...
use super::types::xdg_activation::{ActivationInput, XdgActivationTokenData};
...
pub struct Window {
    ...
    /// Input on the initiating surface, shared by windows on this event loop.
    activation_input: Arc<Mutex<Option<ActivationInput>>>,
...
impl Window {
    pub(crate) fn new(
        ...
        Ok(Self {
            ...
            queue_handle,
            xdg_activation,
            activation_input: state.activation_input.clone(),
...
impl Window {
    ...
    pub fn focus_window(&self) {
        let Some(activation) = self.xdg_activation.as_ref() else {
            return;
        };
        if self.has_focus() {
            return;
        }
        let Some(input) = self.activation_input.lock().unwrap().take() else {
            return;
        };
        if !input.is_valid() {
            return;
        ...
        let data = XdgActivationTokenData::Focus((
            self.surface().clone(),
            Arc::downgrade(&self.window_requests),
            Instant::now(),
        ));
        let token = activation.get_activation_token(&self.queue_handle, data);
        token.set_serial(input.serial, &input.seat);
        token.set_surface(&input.surface);
        token.commit();
        self.event_loop_awakener.ping();
    }
