...
use crate::platform_impl::wayland::types::xdg_activation::ActivationInput;
...
impl Dispatch<WlKeyboard, KeyboardData, WinitState> for WinitState {
    fn event(
        ...
        // Only compositor input carries an activation serial; synthetic key repeats do not.
        if let WlKeyboardEvent::Key { serial, .. } = &event {
            if let Some(window_id) = *data.window_id.lock().unwrap() {
                if let (Some(window), Some(source)) = (
                    state.windows.get_mut().get(&window_id),
                    state.window_requests.get_mut().get(&window_id),
                ) {
                    *state.activation_input.lock().unwrap() = Some(ActivationInput::new(
                        *serial,
                        data.seat.clone(),
                        window.lock().unwrap().surface().clone(),
