...
impl<'a> MvtReaderRef<'a> {
    pub fn new(data: &'a [u8]) -> MvtResult<Self> {
        Self::with_decode_options(data, &buffa::DecodeOptions::default())
    }

    /// Decode borrowed data with allocation limits enforced before creating views.
    pub fn with_decode_options(data: &'a [u8], options: &buffa::DecodeOptions) -> MvtResult<Self> {
        let tile = options.decode_view::<TileView<'a>>(data)?;
