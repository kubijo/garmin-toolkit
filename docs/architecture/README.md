# Architecture

## Ownership and execution

- [Repository layout](repository.md)
- [Rust workspace and service contracts](workspace.md)
- [Application targets](targets.md)
- [Build system](build-system.md)
- [Desktop packaging](desktop-packaging.md)

## Data and devices

- [Data and provenance](data.md)
- [Storage](storage.md)
- [Connectivity and manifest parsing](connectivity.md)
- [Device explorer](device-explorer.md)

## Presentation

- [Shared presentation](presentation.md)
- [Application windows](application-windows.md)
- [Developer tools and logs](developer-tools.md)
- [Visual language](visual-language.md)
- [Activity workspace and map rendering](activity-map.md)

These documents describe current contracts. [Plans](../plans/README.md) own unfinished work;
[research](../research/README.md) records validation and its limits.
