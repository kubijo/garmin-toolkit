...
1. Verify mounted GIO, raw MTP, and filesystem capacity on the watch and Edge. Check storage identity, units, read-only
   media, disconnects, missing attributes, and refresh. Capacity queries must not crawl device files.
...
[Production CLI map maintenance](mounted-device-updates.md) owns transaction checks and hardware write evidence.
[Device expansion](device-expansion-and-publication.md) owns optional battery and transport-health research. Delete this
plan once every claimed client displays refreshed state and hardware results establish each supported adapter.
